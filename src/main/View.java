...
private void drawNodeStats() {
	...
	for (FSys.Node n : FSys.nodes) {
		...
		p5.text(n.verlet.getWeight(), 200, 0);
		p5.text(n.behavior.getRadius(), 250, 0);
		p5.text(n.behavior.getStrength(), 300, 0);
	} p5.noFill();
	p5.popMatrix();
}
private void shitballs() {
	for (FSys.Node n : FSys.nodes) {
		p5.fill(0xffcc0000);
		p5.text(n.id, 0, 0);
		p5.text(n.name, n.verlet.x, 0);
		p5.text((int) n.size + "m²", 60, 0);
		p5.noFill();
		p5.text("[ weight: " + n.verlet.getWeight() + " ] [ area: " + n.verlet.getWeight()
				        + " ] [ rad: " + App.DF3.format(n.radius) + " ]", 310, p5.height - 4);
	}
}
private void drawVerletParticles(App app) {
		p5.fill(Color.PHYS_PTCL);
		for (VerletParticle2D p : app.getPSYS().getPhysics().particles) {
			p5.ellipse(p.x, p.y, 4, 4);
		} p5.noFill();
}
private void drawVerletSprings(App app) {
		p5.stroke(Color.PHYS_SPR);
		for (VerletSpring2D s : app.getPSYS().getPhysics().springs) { p5.line(s.a.x, s.a.y, s.b.x, s.b.y); }
		p5.noStroke();
}
