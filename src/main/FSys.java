...
public void update(App app) {
	if ((App.UPDATE_PHYSVAL) && (App.SCALE != 0)) {
		...
		for (Relation r : relations) {
			...
			float l = (((na.radius + nb.radius) * App.SCALE) / 2) * App.SPR_SCALE;
			app.getPSYS().getPhysics().getSpring(na.verlet, nb.verlet).setRestLength(l);
		}
	}
}
public void clear() {
...
private void drawInfo(ToxiclibsSupport gfx) {
	...
	for (Node n : nodes) {
		...
		pg.text(n.verlet.getWeight(), 200, 0);
		pg.text(n.behavior.getRadius(), 250, 0);
		pg.text(n.behavior.getStrength(), 300, 0);
	} pg.noFill();
	pg.popMatrix();
	pg.pushMatrix();
	pg.translate(1000, 600);
	pg.text(nodes.size(), 0, 0);
	pg.popMatrix();
}
private void drawNodes(ToxiclibsSupport gfx) {
	...
	for (Node n : nodes) {
		...
		pg.text("n_rad " + n.radius, n.verlet.x + n.radius, n.verlet.y);
		pg.stroke(0xff666666);
		pg.ellipse(n.getPos().x, n.getPos().y, n.behavior.getRadius(), n.behavior.getRadius());
		pg.text("nb_rad " + n.behavior.getRadius(), n.verlet.x + n.behavior.getRadius(), n.verlet.y + 10);
		pg.noFill();
...
public static class Node {
	...
	@XmlTransient
	public float radius = (float) (Math.sqrt(size / Math.PI) * App.NODE_SCALE) * App.SCALE;
	@XmlTransient
	...
	@XmlTransient
	public Particle particle = new Particle(new Vec2D(x, y), radius, name, id, size);
...
//		public int getColor() {return color;}
	...
	public void draw(ToxiclibsSupport gfx) {
		...
		pg.ellipse(verlet.x, verlet.y, (radius * App.SCALE)+10,( radius * App.SCALE));
